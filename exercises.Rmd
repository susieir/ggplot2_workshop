...
```{r}
data("faithful")
# Basic scatterplot - with global mapping
ggplot(data = faithful, 
       mapping = aes(x = eruptions, y = waiting)) + 
  geom_point()
# Data and mapping can be given both as global (in ggplot()) or per layer

# With layer specific mapping (no global data at all)
ggplot() + 
...
```{r}
# Don't usually need the mapping
# Usually you have global data and some layer specific mapping
# Inside mapping - can also map other aesthetics, such as color
# Automatically get legend - takes names from mapping
ggplot(faithful) + 
...
```{r}
# Instead of mapping color - you can set it (i.e. to a fixed value)
# If this is the case - should be outside of the aesthetic - not a mapping
ggplot(faithful) + 
...
```{r}
# Hist - only need x value, ggplot calculates the y for you
ggplot(faithful) + 
...
```{r}
# Order or arguments within the plus sign matters. Layers are stacked in order of code appearance
# Points on top of density plot
ggplot(faithful, aes(x = eruptions, y = waiting)) + 
  geom_density_2d() + # Shown as contours
  geom_point()
```
...
Histogram - default position is stack
When changed to identity - moves behind
...
```{r}
# Modifies to percentage of the whole
# Count is being calculated by the stat
ggplot(mpg) + 
...
ggplot(mpg) + 
  ...
  stat_summary(aes(x = class, y = hwy), fun = median, geom= 'point', color = 'red', size = 5)
```
...
# Won't penalise wide plots - it will just make the plot area bigger
# Widths can also be a unit vector widths = unit(c(5, 1, 1), c("cm", "null", "null"))
...
```{r}
ggplot(mpg) + 
  geom_point(aes(x = displ, y = hwy, group = seq_len(nrow(mpg)))) # Each row a distinct observation - stops points moving between points
+ 
